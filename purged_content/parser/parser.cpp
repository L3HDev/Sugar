...
void Parser::statement(std::shared_ptr<Node> n)
{
    ...
    else if (lex.at(tc + 1).tokenType != TokenType::SEMICOLON)
    {
        expression(res);
    }
    else
...
void Parser::eqop(std::shared_ptr<Node> n)
{
    ...
    res = n->push<EqOp>();
    binop1(res);
    while (tc + 1 < lex.size() && is_compare_type_token(lex.at(tc + 1).tokenType))
    {
        ++tc;
...
void Parser::string(std::shared_ptr<Node> n)
{
}

bool Parser::is_compare_type_token(TokenType t)
{
    return t == TokenType::DEQUAL ||
           t == TokenType::NEQUAL ||
           t == TokenType::LESS ||
